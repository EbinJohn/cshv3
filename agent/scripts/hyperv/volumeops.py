...
# Copyright 2012 Citrix / Donal Lafferty
...
class VolumeOps(baseops.BaseOps):
    ...
    def __init__(self):
        ...
        #todo: make this a config file setting
        #The number of times we retry on attaching volume
        self._attaching_volume_retry_count = 10
        #todo: make this a config file setting
        #The seconds to wait between an volume attachment attempt
        self._wait_between_attach_retry = 5
    ...
    def detach_volume(self, instance_name, mountpoint):
        ...
        LOG.debug(_("Detach_volume at %(mountpoint)s on %(instance_name)s,")
                   % locals())
        #Getting the mounted disk list
        disk_list = self._conn.query(
                "SELECT * FROM MSVM_ResourceAllocationSettingData \
                WHERE ResourceSubType LIKE 'Microsoft Virtual Hard Disk'")
        physical_disk = 0
        for phydisk in disk_list:
        ...
        LOG.debug(_("Physical disk to detach is: %s"), physical_disk)
        vms = self._conn.MSVM_ComputerSystem(ElementName=instance_name)
        vm = vms[0]
